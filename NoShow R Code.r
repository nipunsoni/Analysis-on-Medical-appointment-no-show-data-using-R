...
NoShowDataUpdated = NoShowDataUpdated[NoShowDataUpdated$Age > 0 & NoShowDataUpdated$Age < 100,]
...
#Logistic
NS.LR = glm(No.show ~ Age + SMS_received + Scholarship, data = trainData, family = binomial(link = "logit"))
summary(NS.LR)
...
#Ridge
ridge.mod <- glmnet(x1, y, alpha=0, nlambda=1000, lambda.min.ratio=0.0001)
ridge.mod$lambda[20]
coef(ridge.mod)[,20]

set.seed(1)
cv.out <- cv.glmnet(x1, y, alpha=0, nlambda=1000, lambda.min.ratio=0.0001)
plot(cv.out)
